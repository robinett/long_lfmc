...
while [ "$current_date" != "$end_date" ]; do
    ...
    for var in "${vars[@]}"; do
        file_path="/scratch/users/trobinet/long_lfmc/trent_datasets/daymet/daymet_download/${var}/${yr}/${mn}/${var}_${yr}_${mn}_${day}.nc"
        if [ ! -f "$file_path" ]; then
	        wget -O "${file_path}" "https://thredds.daac.ornl.gov/thredds/ncss/grid/ornldaac/2129/daymet_v4_daily_${region}_${var}_${yr}.nc?var=lat&var=lon&var=${var}&north=${north}&west=${west}&east=${east}&south=${south}&horizStride=1&time_start=${yr}-${mn}-${day}T12:00:00Z&time_end=${yr}-${mn}-${day}T12:00:00Z&timeStride=1&accept=netcdf"
