...
daymet_archive_path="${registry_values[3]}"
nlcd_raw_path="${registry_values[4]}"
...
record = {
    ...
    'bootstrap': {
        ...
        'daymet_archive': os.environ['BOOTSTRAP_DAYMET'],
...
update_status_env() {
    ...
    export BOOTSTRAP_DAYMET="${bootstrap_daymet}"
...
for year in "${candidate_years[@]}"; do
    ...
    if python3 "${daymet_update_script}" --year "${year}" --check_only; then
        :
    else
        daymet_reason="daymet_unavailable"
    fi
    if python3 "${nlcd_update_script}" --year "${year}" --check_only; then
        :
    else
...
if ensure_on_scratch "${daymet_archive_path}" "Daymet archive zarr" "0"; then bootstrap_daymet="ready"; else bootstrap_daymet="missing"; fi
if ensure_on_scratch "${nlcd_raw_path}" "NLCD raw zarr" "0"; then bootstrap_nlcd_raw="ready"; else bootstrap_nlcd_raw="missing"; fi
...
if ensure_on_scratch "${climate_path}" "climate zone dataset" "0"; then bootstrap_climate="ready"; else bootstrap_climate="missing"; fi
...
if [[ "${bootstrap_daymet}" != "ready" || "${bootstrap_nlcd_raw}" != "ready" || "${bootstrap_nlcd_annual}" != "ready" || "${bootstrap_static}" != "ready" || "${bootstrap_climate}" != "ready" || "${bootstrap_grid}" != "ready" || "${bootstrap_models}" != "ready" ]]; then
...
for year in "${passed_years[@]}"; do
    ...
    source "${process_env}"
    if python3 "${daymet_update_script}" --year "${year}"; then
    ...
    if python3 "${nlcd_update_script}" --year "${year}"; then
    ...
    source "${model_env}"
    if UPDATE_YEAR="${year}" SKIP_SOURCE_UPDATE=1 bash "${final_driver_script}"; then
