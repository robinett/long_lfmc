...
sys.path.append(os.path.join(project_root,'lfmc_model','models','rnn'))
sys.path.append(os.path.join(project_root,'lfmc_model','utils'))

#from transformer_model import LFMCTransformer
from transformer_multitask import LFMCTransformer as LFMCTransformerMultitask
...
from transformer_multitask_uncertainty import LFMCTransformer as LFMCTransformerMultitaskUncertainty
from krishna_rnn import LFMCRnn
import plotting
...
def mask_location_data_fast(
    ...
    if len(info) == 0 or len(keep_locs) == 0:
        ...
        empty = slice(0, 0)
        return (short_data[empty], np.nan, static_data[empty], y[empty], source[empty], info.iloc[:0], stratifier[empty],
                short_data, np.nan, static_data, y, source, info, stratifier)
    ...
    return (
        kept_short_data, kept_static_data, kept_y, kept_source, kept_info, kept_stratifier,
        remaining_short_data, remaining_static_data, remaining_y, remaining_source, remaining_info, remaining_stratifier
    )
...
def load_data(center_data_dir):
    ...
    all_center_data = [
        X_short, np.nan, X_static, Y, source, center_info, stratifier
    ]
...
def run_model(
    ...
    for Xsh_b, Xst_b, Y_b, insitu_b in pbar:
        # move data to device
...
def train_fold_k(
    ...
    stratifier = data[6]
    (
        test_short_data,  test_static_data, test_y, test_source, test_info, test_stratifier,
        remaining_short_data, remaining_static_data, remaining_y, remaining_source, remaining_info, remaining_stratifier
    ...
    (
        val_short_data, val_static_data, val_y, val_source, val_info, val_stratifier,
        train_short_data, train_static_data, train_y, train_source, train_info, train_stratifier
    ) = mask_location_data_fast(
        val_locs,
    ...
    for epoch in range(1,max_epochs):
        ...
        # test just for plotting at the end
        (
            _,
            this_test_loss,
        ...
        test_loss_insitu.append(this_test_loss_insitu)
        test_loss_vv.append(this_test_loss_vv)
        test_loss_vh.append(this_test_loss_vh)
        ## get mixture
