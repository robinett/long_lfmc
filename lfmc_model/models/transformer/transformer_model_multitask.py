...
class GaussianHead(nn.Module):
    def __init__(self,d_model,dropout=0.0):
        ...
        self.net = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(d_model//2, 2)  # mean, logvar
        )
    def forward(self,h):
        out = self.net(h)          # [B,2]
        mu, log_var = out[:,0], out[:,1]
        log_var = torch.clamp(log_var, -10, 10)  # avoid overflow
        return mu, log_var
...
class LFMCTransformer(nn.Module):
    def __init__(self, short_input_dim, static_input_dim,
        ...
        self.head_rs = GaussianHead(d_model, dropout)
    def forward(self, short_history, static_features):
        ...
        h_stat = x_enc[:, 0, :]                        # [B,d]
        h = h_met + h_stat
        mu_insitu, log_var_insitu = self.head_insitu(h)  # [B], [B]
        mu_rs, log_var_rs = self.head_rs(h)              # [B], [B]
        y = {
            'mu_insitu': mu_insitu,
            'log_var_insitu': log_var_insitu,
            'mu_rs': mu_rs,
            'log_var_rs': log_var_rs
        }
        return y